<?php
// Store the content of the parent post
$microsite_post = $post;

if (isset($_GET['detail'])) {
    $detail = true;
    $detail_id = $_GET['detail'];
    $detail_post = get_post($detail_id);
    $post = $detail_post;
    global $post;
    setup_postdata( $post );
}
            ...
            <article class="">
                ...
                <header class="l-flex-col banner microsite-details-banner" style="background-image: url('<?php echo $banner_image; ?>');">
                    ...
                    <?php wp_reset_postdata(); ?>
                        <a class="breadcrumbs__linkhref" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                        /
                        <?php
                            $post = $detail_post;
                            global $post;
                            ...
                            <?php else: ?>
                              <?php
                                ...
                                if ($tags) {
                                  ...
                                  $args = array(
                                    'tag' => $tagsList,
                                    'post__not_in' => array($post->ID, $microsite_post->ID),
                                    'posts_per_page' => 3,
                                    'orderby' => 'rand'
                                  );
                ...
                <?php if( have_rows('featured_resources') ): ?>
                    ...
                    ?>
                    <a href="?detail=<?php echo $resource->ID; ?>" class="microsite-featured-resource" style="background-image: url('<?php echo $resource_image; ?>');">
                        <div class="featured-resource-content">
                            ...
                            <div href="?detail=<?php echo $resource->ID; ?>" class="btn-transparent"><?php echo $resource_label; ?></div>
                        </div>
                    </a>
                    <?php endwhile; ?>
                    </section>
